...
opt_run_autogen() {
    ...
    if [ $auto_autogen = "1" ] ; then
        if [ ! -e "${package}${version}/autogen.sh" ] ; then
            ...
            print_info "Found autogenerated.cascade for ${progname%.*} => no need to run ./autogen.sh"
        fi
    fi
...
opt_run_configure()
{
    ...
    if [ $auto_config = "1" ] ; then
        if [ ! -e "${package}${version}/configure" ] ; then
            ...
            print_info "Found configured.cascade for ${progname%.*} => no need to run ./configure"
        fi
    fi
