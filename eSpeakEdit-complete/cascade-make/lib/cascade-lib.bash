...
default_toplevel_make_dist()
{
  ...
  local checklist="setup.bash setup.bat perllib $PACKAGE_OS lib $*"
...
toplevel_make_minimal()
{
  ...
  local checklist="devel.bash setup.bash_old setup.bat_old $PACKAGE_OS/lib $PACKAGE_OS/bin $PACKAGE_OS/include $*"
  local distlist=""
...
run_installclean()
{
  local fulldir="$package_home/$PACKAGE_OS"
  echo ""
...
print_usage()
{
  echo "$0 [untar|configure|compile|install|clean|distclean|tarclean|makedist|makeminimal]+"
  echo "  or"
